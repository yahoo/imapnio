...
import com.lafaspot.imapnio.async.netty.ImapClientConnectHandler;
...
public class ImapAsyncClient {
    ...
    /** Socket connect timeout key. */
    public static final String CONFIG_CONNECTION_TIMEOUT_KEY_MILLIS = "mail.imap.connectiontimeout";

    /** IMAP response read from server timeout key. */
    ...
    public static final int DEFAULT_IMAP_TIMEOUT_MILLIS = 10000;
     ...
     */
    public ImapFuture<ImapAsyncSession> createSession(@Nonnull final URI serverUri, @Nonnull final ImapAsyncSessionConfig config,
            @Nullable final InetSocketAddress localAddress, @Nullable final List<String> sniNames) {
        // ------------------------------------------------------------
        // check if required properties are present
        final int connectionTimeMillis = (config.getConnectionTimeoutMillis() <= 0) ? DEFAULT_CONNECTION_TIMEOUT_MILLIS
                : config.getConnectionTimeoutMillis();
        final int readTimeMillis = (config.getReadTimeoutMillis() <= 0) ? DEFAULT_IMAP_TIMEOUT_MILLIS : config.getReadTimeoutMillis();
