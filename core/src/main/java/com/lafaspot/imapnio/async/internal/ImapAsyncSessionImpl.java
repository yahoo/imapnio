...
import com.lafaspot.imapnio.async.netty.ImapClientCommandRespHandler;
import com.lafaspot.imapnio.async.netty.ImapCommandChannelEventProcessor;
...
public class ImapAsyncSessionImpl implements ImapAsyncSession, ImapCommandChannelEventProcessor, ChannelFutureListener {
    ...
    /** Flag to indicate compression is sent and pending to server. */
    private boolean isCompressionRequestPending;
    ...
    public ImapAsyncSessionImpl(@Nonnull final Channel channel, @Nonnull final Logger logger, final int sessionId, final ChannelPipeline pipeline) {
        ...
        this.isCompressionRequestPending = false;
    ...
    public <T> ImapFuture<ImapAsyncResponse> startCompression() throws ImapAsyncClientException, SearchException, IOException {
        ...
        // set it to true only after submitted without failure
        isCompressionRequestPending = true;
        return future;
    }
    ...
    private void requestDoneWithException(@Nonnull final ImapAsyncClientException cause) {
        ...
        isCompressionRequestPending = false; // always shut it off
    ...
    public <T> void handleChannelResponse(@Nonnull final IMAPResponse serverResponse) {
        ...
        if (serverResponse.isContinuation()) {
            ...
            if (isCompressionRequestPending) {
                isCompressionRequestPending = false; // no longer pending after server responds
